...
def get_pizza_title(driver, replace_quotes=True):
    title = driver.find_element(By.CLASS_NAME, 'product_title').text.lower()
    return sub(r'[«»]', '"', title) if replace_quotes else title


def get_pizza_price(driver):
    price = driver.find_element(By.CSS_SELECTOR, "p.price bdi").text[:-1]
    return str_to_float(price)


def find_doping_menu(driver):
    return Select(driver.find_element(By.ID, "board_pack"))
...
def send_keys_to_input_form(driver, key):
    counter = driver.find_element(By.CSS_SELECTOR, "input[type='number']")
    counter.clear()
...
def get_add_pizza_notification(driver):
    return driver.find_element(By.CLASS_NAME, "woocommerce-message")
...
def go_to_cart_via_notification(driver):
    ...
    notification.find_element(By.CLASS_NAME, "wc-forward").click()
